...
Googleカレンダーの予定名に含まれる絵文字・キーワードを検知し、飲み会の日の夜間や、オンライン・オフライン勉強会の前後時間を自動でブロックする、Googleスプレッドシート紐づけ型のGoogle Apps Scriptです。
...
| --- | --- | --- |
| 飲み会 | `🍺` | 対象日の18:00〜24:00 |
| オンライン勉強会 | `💻` | 元予定の開始前0.5時間・終了後0.5時間 |
| オフライン勉強会 | `🏫` | 元予定の開始前1.5時間・終了後1.5時間 |
...
- 飲み会は判定開始時刻をまたぐ予定も対象化し、その日の固定時間帯をロック
- オンライン・オフライン勉強会は開始前・終了後の時間を個別設定
- 元の勉強会予定と重ならない事前・事後ブロックを作成
...
3. 飲み会、オンライン勉強会、オフライン勉強会の判定文字列と時間を確認し、「設定を保存」を押します。
...
デフォルトでは、予定名に `💻` を含む予定の開始前0.5時間と終了後0.5時間をブロックします。

たとえば `AI活用オンライン勉強会 💻` が19:00〜21:00の場合、次の2予定を作成します。

- 18:30〜19:00 `🔒 事前ブロック｜オンライン勉強会`
- 21:00〜21:30 `🔒 事後ブロック｜オンライン勉強会`

### オフライン勉強会の前後バッファ

デフォルトでは、予定名に `🏫` を含む予定の開始前1.5時間と終了後1.5時間をブロックします。移動、受付、会場準備、終了後の移動や振り返りに利用できます。

たとえば `東京オフライン勉強会 🏫` が19:00〜21:00で、開始前2時間・終了後1時間へ変更した場合、次の2予定を作成します。

- 17:00〜19:00 `🔒 事前ブロック｜オフライン勉強会`
- 21:00〜22:00 `🔒 事後ブロック｜オフライン勉強会`

前または後を0時間にすると、その側の予定は作成しません。1件の予定にオンラインとオフライン両方の判定文字列がある場合は、移動時間を考慮してオフライン設定を優先します。
...
| オンライン勉強会を有効化 | オン | オンラインルールを使用するか |
| オンライン判定文字列 | `💻` | オンライン勉強会を判定する絵文字・キーワード |
| オンライン開始前／終了後 | `0.5`／`0.5`時間 | 元予定の前後へ作るバッファ |
| オフライン勉強会を有効化 | オン | オフラインルールを使用するか |
| オフライン判定文字列 | `🏫` | オフライン勉強会を判定する絵文字・キーワード |
| オフライン開始前／終了後 | `1.5`／`1.5`時間 | 移動・準備・振り返り用のバッファ |
...
- 曜日別の対象日数
- 曜日別件数の棒グラフ
- 飲み会・オンライン・オフラインの種別別作成件数と比較グラフ
- 勉強会の元予定名、種別、事前／事後、ブロック開始・終了一覧
...
サイドバーの「ダミーデータを作成」を押すと、今日から60日後までに20〜30件の予定を作成します。飲み会、オンライン勉強会、オフライン勉強会を最低1件ずつ含み、残りはランダムです。
...
飲み会ロックの削除対象になるのは、次の両方を満たす予定だけです。

1. 予定名が現在設定されている自動生成予定名と完全一致する
2. 説明欄に `SYSTEM_EVENT_ID: CALENDAR_EMOJI_TIME_LOCK` が含まれる

勉強会バッファは、固定された事前・事後タイトルとシステム識別子の両方が一致する場合だけ削除します。説明欄にはさらに次の追跡情報を保存します。

- `BUFFER_RULE`: オンラインまたはオフライン
- `BUFFER_PHASE`: 事前または事後
- `SOURCE_EVENT_ID`: 元予定ID
- `SOURCE_EVENT_START`: 元予定の開始日時
- `SOURCE_EVENT_TITLE`: 元予定名
